package com.example.reactive_redis_template;
...
public class ReactiveRedisTemplateApplication implements ApplicationRunner {

	public static void main(String[] args) {
		SpringApplication.run(ReactiveRedisTemplateApplication.class, args);
	}
		...
		Flux.fromIterable(list)
				...
				.flatMap(
						c->{
							...
							return ops.put(KEY,c.getName(),c.getPrice().toString());
						}
				).doOnComplete(
				()->log.info("set ok")
				).concatWith(redisTemplate.expire(KEY, Duration.ofMinutes(1))).doOnComplete(
				()->log.info("expire ok")
		).onErrorResume(e->{
