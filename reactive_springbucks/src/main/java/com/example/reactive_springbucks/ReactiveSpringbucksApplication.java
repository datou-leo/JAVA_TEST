...
import com.example.reactive_springbucks.converter.MoneyReadConverter;
import com.example.reactive_springbucks.converter.MoneyWriteConverter;
...
public class ReactiveSpringbucksApplication extends AbstractR2dbcConfiguration {
	...
	public ReactiveRedisTemplate<String, Coffee> reactiveRedisTemplate(ReactiveRedisConnectionFactory factory){
		...
		RedisSerializationContext.RedisSerializationContextBuilder<String,Coffee> builder =RedisSerializationContext.newSerializationContext(keySerializer);

				RedisSerializationContext<String,Coffee> ctx=builder.value(valueSerializer).build();
